...
if __name__ == '__main__':
    ...
    for content in content_divs:
        if content.startswith("//zhuanlan.zhihu.com"):
            ...
            if content not in MyPlaywright.p_urls:
                MyPlaywright.p_urls.append(content)
        ...
        if answer_index != -1:
            ...
            if content not in MyPlaywright.question_urls:
                MyPlaywright.question_urls.append(content)
    print(MyPlaywright.question_urls,MyPlaywright.p_urls)
    ...
    with sync_playwright() as playwright:
        ...
        for url in MyPlaywright.question_urls:
            pagedata={}
            page = context.new_page()
            try:
                page.goto(url)
            ...
            tree = etree.HTML(content)
            # 使用XPath表达式选择元素
            elements = tree.xpath('//div[@class="QuestionHeader"]//span[starts-with(@class, "RichText") and @itemprop="text"]/p')
            # 打印所有选中元素的文本内容
            question_text=""
            ...
            MyPlaywright.datas_pagedata.append(pagedata)
        ...
        # ---------------------
        context.close()
        print(MyPlaywright.datas_pagedata)
